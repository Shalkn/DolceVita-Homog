...
//***EVENTS */
  ...
  //Para a página password_reset.html
  doc.addEventListener("DOMContentLoaded", function() {
  ...
  //Script para alternar entre os formulários página login
  doc.addEventListener('DOMContentLoaded', function() {
    // Esconde o formulário de cadastro ao carregar a página
    doc.getElementById('formUsers').classList.add('d-none');
    //doc.getElementById('formesquecer').classList.add('d-none');
    
    // Quando o link "Registre-se aqui!" for clicado, alterna os formulários
    doc.getElementById('registerLink').addEventListener('click', function(e) {
      e.preventDefault();
      
      // Oculta o formulário de login e exibe o de cadastro
      doc.getElementById('formlogin').classList.add('d-none');
      doc.getElementById('formUsers').classList.remove('d-none');
    });

    doc.getElementById('contaexist').addEventListener('click', function(e) {
      e.preventDefault();
      
      // Oculta o formulário de login e exibe o de cadastro
      doc.getElementById('formlogin').classList.remove('d-none');
      doc.getElementById('formUsers').classList.add('d-none');
    });
    
    //doc.getElementById('esquecersenha').addEventListener('click', function(e) {
    //  e.preventDefault();
      
      // Oculta o formulário de login e exibe o de cadastro
   //   doc.getElementById('formlogin').classList.add('d-none');
   //   doc.getElementById('formesquecer').classList.remove('d-none');
  //  });

    doc.getElementById('returnlogin').addEventListener('click', function(e) {
      e.preventDefault();
...
//Confirmar se o usuário deseja excluir o produto
  ...
  function ajax(form){
    if(doc.querySelector(form)){
      let forms = doc.querySelector(form);
      ...
      function sendForm(event){
        ...
        ajax.onreadystatechange = function(){
          if(ajax.status === 200 && ajax.readyState ===4){
            ...
            result.classList.add('alert-succes');
      ...
      forms.addEventListener('submit', sendForm, false)
    }
  }


ajax('#formCreateProdutos');
ajax('#formEditUsers');
